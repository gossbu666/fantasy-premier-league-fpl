...
# 1) โหลด base data
df = pd.read_csv("data/processed/FWD_data.csv")
df["season"] = df["season"].astype(str)

print(f"Loaded FWD_data.csv: {len(df)} rows")
...
# 5) เลือกคอลัมน์ (ใช้คอลัมน์ที่มีจริง)
base_feature_cols = [
...
out.to_csv("data/processed/FWD_features_enhanced_safe.csv", index=False)
print("✅ Saved season-safe FWD features")
